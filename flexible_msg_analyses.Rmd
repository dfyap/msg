...
df <- read.csv('data/msgclause.csv')
df$Sub <- factor(paste(df$Sub, df$experiment, sep ='_')) # tag experiment to sub
names(df)[names(df) == 'direction'] <- 'gestureDirection' # rename gesture directions

# Which variables do we care about?
lang.type <- 'Input.Lang' # 'Output.Language'
direction.var <- 'StoryDirection' # 'ClauseDirection'

# exclude non-gestures and emblems
msg <- df[which(df$storyGesture != "" 
                 & df$storyGesture != "ADAPTOR"
                 & df$storyGesture != 'EMBLEM'
                 & df$storyGesture != "NA "),] 
...
# select subset of intest
# msg <- subset(msg, Target.NonTarget == 'NonTarget') 
#df$Output.Language <- factor(paste(df$Output.Language, "SL", sep ='')) # tag experiment to sub
#table(df$Output.Language)
...
clusterExport(cl, c('shuffle', 'Congruity', 'how', 'msg'))
...
We performed three separate permutation tests (k=10,000, each) to compare the pairwise differences in gesture congruity between each pair of visibility conditions. We randomly permuted the story directions within each participant, and computed the pairwise differences in congruity. 
...
### Differences in congruity between input language types

We performed a permutation test to determine whether the mean proportion of congruity differs between the different types of input language. We computed the variance of the proportion congruity between the three groups, and then randomly shuffled the input language label for each trial ($k = 10,000$).
...
``` {r allGestureDescriptives, cache=FALSE}
clusterExport(cl, c('msg','direction.var', 'BootSingleDirection'))
...
counts <- summary(df$storyGesture)[1:6]
...
counts <- summary(df$gestureDirection)[1:11] # exclude NAs
bar.names <- names(counts)
...
# Calculate empirical and permuted congruity in each language type and storydirection 
...
plot(NULL, NULL, ylim = c(0,1), xlim = c(1, 14),
     xlab='Story', xaxt='n', ylab='Congruity')
...
# do a 'dictionary' of arrows for their labels
...
Across all story types and directions, participants gestured in a direction congruent with the direction of the story `r round(Congruity(msg), 3) * 100` % of the time. This proportion of congruence was higher than would be expected by chance `r ReportPermute(Congruity(msg), perm.stat)`.

``` {r visibilityChanceCongrutiy, cache=FALSE} 
clusterExport(cl, c('GroupVar', 'PairwiseDiffHelper'))

# Congruity across visibility conditions
empVar <-GroupVar(msg,'experiment') # empirical value
permVar.dist <- OnewayPermTest(msg,'experiment') # chance distribution
...
permExp1v3Pair <- WithinSubsCongPairwisePermute(msg,'experiment','MSG1','MSG3') 

empExp1v4Pair <- PairwiseDiffHelper(msg,'experiment', 'MSG1','MSG4')
permExp1v4Pair <- WithinSubsCongPairwisePermute(msg,'experiment','MSG1','MSG4')

empExp3v4Pair <- PairwiseDiffHelper(msg,'experiment', 'MSG3','MSG4')
permExp3v4Pair <- WithinSubsCongPairwisePermute(msg,'experiment','MSG3','MSG4') 
...
In this section, we'll want to include the 'one-way ANOVA' across all three visibility conditions, as well as pairwise permutation tests. Also, include tests of whether congruity is greater than chance in each visibility condition.We randomly permuted the visibility conditions between each participant, and computed the variance of the mean congruity across visibility conditions. The observed variance was `r round(empVar,4)*100`%, which was significantly greater than the permuted chance variance `r ReportOnewayPerm(empVar, permVar.dist)`.

We performed three separate permutation tests (k=10,000, each) between each pair of visibility conditions. We randomly permuted the story directions within each participant, and computed the pairwise differences in congruity. Congruity was higher in the no-barrier than in barrier-only condition (`r empExp1v3Pair*100`%, chance rate = `r ReportPermute(empExp1v3Pair,permExp1v3Pair)`, higher in no-barrier than in the barrier-with-blindfold condition (`r empExp1v4Pair*100`%, chance rate = `r ReportPermute(empExp1v4Pair,permExp1v4Pair)`, and higher in barrier-with-blindfold than the barrier-only condition (`r empExp3v4Pair*100`%, chance rate = `r ReportPermute(empExp3v4Pair,permExp3v4Pair)`.

We performed within-subjects permutations (k=10,000) to test whether congruity within each visibility condition was greater than would be expected by chance. Participants produced congruent gestures more often than would be expected by chance in all three visibility conditions: no-barrier condition: `r round(empExp1, 3) * 100`% congruent  (chance rate = `r ReportPermute(empExp1, permExp1.dist)`); barrier condition: `r round(empExp3, 3) * 100`% congruent (chance rate =  `r ReportPermute(empExp3, permExp3.dist)`) , and barrier-and-blindfold condition: `r round(empExp4, 3) * 100`% congruent (chance rate =  `r ReportPermute(empExp4, permExp4.dist)`).
...
In all four story directions, participants gestured congruently with the story direction more often than would be expected by chance. Upward stories had a `r round(emp.up, 3) * 100`% observed rate of congruity (chance rate = `r ReportBootstrap(emp.up,boot.up)`%, `r ReportOddsRatio(msg,'up')`. Downward stories had a `r round(emp.down, 3) * 100`% observed rate of congruity (chance rate = `r ReportBootstrap(emp.up,boot.down)`%, `r ReportOddsRatio(msg,'down')`. Leftward stories had a `r round(emp.left, 3) * 100`% observed rate of congruity (chance rate = `r ReportBootstrap(emp.left,boot.left)`%, `r ReportOddsRatio(msg,'left')`. Rightward stories had a `r round(emp.right, 3) * 100`% observed rate of congruity (chance rate = `r ReportBootstrap(emp.right,boot.right)`%, `r ReportOddsRatio(msg,'right')`.

### Congruity across literal, metaphorical, and non-spatial stories

``` {r OneWayPermLanguageTypes, cache=FALSE} 

permVar.dist <- OnewayPermTest(msg, 'Input.Lang')
...
We randomly permuted the language types within each participant, and computed the variance of the mean congruity across language types. The observed variance was `r round(GroupVar(msg,'Input.Lang'),4)*100`%, which was significantly greater than the permuted chance variance `r ReportOnewayPerm(GroupVar(msg,'Input.Lang'), permVar.dist)`.

``` {r pairwisediffChanceCongrutiyLanguageTypes, cache=FALSE}

permMLPair <- WithinSubsCongPairwisePermute(msg,'Input.Lang','MSL','LSL') 
permNLPair <- WithinSubsCongPairwisePermute(msg,'Input.Lang','NSL','LSL')
permMNPair <- WithinSubsCongPairwisePermute(msg,'Input.Lang','MSL','NSL') 

```

### Pairwise differences in congruity between language types
We performed three separate permutation tests (k=10,000, each) between each pair of language types. We randomly permuted the story directions within each participant, and computed the pairwise differences in congruity. Congruity was higher in LSL than in MSL (`r PairwiseDiffHelper(msg,'Input.Lang','MSL','LSL')`, chance rate = `r ReportPermute(PairwiseDiffHelper(msg,'Input.Lang','MSL','LSL'),permMLPair)`, higher in NSL than in LSL (`r PairwiseDiffHelper(msg,'Input.Lang','NSL','LSL')`, chance rate = `r ReportPermute(PairwiseDiffHelper(msg,'Input.Lang','NSL','LSL'),permNLPair)`, and higher in NSL than in MSL (`r PairwiseDiffHelper(msg,'Input.Lang','MSL','LSL')`, chance rate = `r ReportPermute(PairwiseDiffHelper(msg,'Input.Lang','MSL','NSL'),permMNPair)`.

``` {r chanceCongruityLanguageTypes, cache=FALSE}

permLSL.dist <- WithinSubsCongPermute(subset(msg,Input.Lang == 'LSL'))
permMSL.dist <- WithinSubsCongPermute(subset(msg,Input.Lang == 'MSL'))
permNSL.dist <- WithinSubsCongPermute(subset(msg,Input.Lang == 'NSL'))  

```

### Congruity in each language type
We performed within-subjects permutations (k=10,000) to test whether congruity within each language type was greater than would be expected by chance. Participants produced congruent gestures more often than would be expected by chance in all three language types: LSL: `r round(Congruity(subset(msg, Input.Lang=='LSL')), 3) * 100`% congruent  (chance rate = `r ReportPermute(Congruity(subset(msg, Input.Lang=='LSL')), permLSL.dist)`); MSL: `r round(Congruity(subset(msg, Input.Lang=='MSL')), 3) * 100`% congruent (chance rate =  `r ReportPermute(Congruity(subset(msg, Input.Lang=='MSL')), permMSL.dist)`) , and NSL: `r round(Congruity(subset(msg, Input.Lang=='NSL')), 3) * 100`% congruent (chance rate =  `r ReportPermute(Congruity(subset(msg, Input.Lang=='NSL')), permNSL.dist)`).

``` {r chanceCongruityClauseTypes, cache=FALSE}

# Pairwise difference in congruity between clauses
permClausePair <- WithinSubsCongPairwisePermute(msg,'Target.NonTarget','Target','NonTarget')

# Within-sub congruity in each type of clause
permTarget.dist <- WithinSubsCongPermute(subset(msg,Target.NonTarget == 'Target'))
permNonTarget.dist <- WithinSubsCongPermute(subset(msg,Target.NonTarget == 'NonTarget'))

```

### Congruity according to the presence of a directional meaning
Congruity was higher in Target than in Non-target clauses (`r PairwiseDiffHelper(msg,'Target.NonTarget','Target','NonTarget')`, chance rate = `r ReportPermute(PairwiseDiffHelper(msg,'Target.NonTarget','Target','NonTarget'),permClausePair)`.Participants produced congruent gestures greater than would be expected by chance in both types of clauses: Target clauses: `r round(Congruity(subset(msg, Target.NonTarget=='Target')), 3) * 100`% congruent  (chance rate = `r ReportPermute(Congruity(subset(msg, Target.NonTarget=='Target')), permTarget.dist)`); Non-target clauses: `r round(Congruity(subset(msg, Target.NonTarget=='NonTarget')), 3) * 100`% congruent  (chance rate = `r ReportPermute(Congruity(subset(msg, Target.NonTarget=='NonTarget')), permNonTarget.dist)`).
